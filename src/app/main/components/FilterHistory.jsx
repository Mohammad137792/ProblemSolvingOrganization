...
export default function FilterHistory({formValues={}, setFormValues=()=>{}, filterType, loadCallback=()=>{}}) {
    ...
    React.useEffect(()=>{
        axios.get(SERVER_URL + "/rest/s1/fadak/entity/UserValues", {
    ...
    const handleSave = (filterValues)=>{
        if(filterValues.filterName){
            ...
            axios.post(SERVER_URL + "/rest/s1/fadak/entity/UserValues", {data : packet}, {
                headers: {'api_key': localStorage.getItem('api_key')}
            }).then(res => {
    ...
    const handleDelete = (option, index)=>()=>{
        dispatch(setAlertContent(ALERT_TYPES.WARNING, "در حال ارسال اطلاعات..."));
        axios.delete(SERVER_URL + `/rest/s1/fadak/entity/UserValues?id=${option.id}`, {
            headers: {'api_key': localStorage.getItem('api_key')},
        }).then(() => {
