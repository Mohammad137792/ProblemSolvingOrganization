...
import NeedAssessment from "../../../tasks/forms/Training/needAssessment/needAssessment";
...
export default function TrainingNeedsAssessment(){
    ...
    React.useEffect(()=>{
        axios.get(SERVER_URL+"/rest/s1/fadak/process/list",{
            ...
            setProcessDefinitionId( res.data.outList.find(i=>i.key==="NeedAssessment").id )
        }).catch(() => {
        });
    },[])
    ...
    return(
        ...
        <>
                {state==='Default' &&
                <NeedAssessment submitCallback={submitCallback}/>
                }
                {state==='StartAnother' &&
                <Box textAlign="center" py={5}>
                    <Button onClick={newOrder} color="secondary" variant="contained">صدور نیازمندی جدید</Button>
                </Box>
                }
        </>
        //     </CardContent>
