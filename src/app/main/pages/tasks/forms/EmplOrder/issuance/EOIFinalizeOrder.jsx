...
export default function EOIFinalizeOrder(props){
    ...
    const {formValues, setFormValues, personnelOrders,verificationList} = props;
    ...
    React.useEffect(()=>{
        if(verificationList){
            setTable1Content(verificationList)
            
        }
        else{
            if(formValues.settingId || formValues.agreementId || formValues.AgreementTypeId) {
           
                axios.get(SERVER_URL + "/rest/s1/emplOrder/emplOrderVerificationLevel"  , {
                    ...
                    const verificationLevels = res.data.verificationLevels;
                    setTable1Content(verificationLevels)
                }).catch(() => {
                    setTable1Content([])
                });
            }
        }
    },[formValues.settingId, formValues.agreementId,formValues.newAgreement,formValues.AgreementTypeId])

    React.useEffect(()=>{
        setFormValues(prevValue => ({
            ...prevValue,
            verificationList: table1Content.sort((a, b) => a.sequence-b.sequence)
        }))
    },[table1Content])
        ...
        <>
            <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                    <Card variant="outlined">
                        <TablePro
                            title="لیست احکام کارگزینی صادر شده / قرارداد های تنظیم شده"
                            columns={table2Cols}
