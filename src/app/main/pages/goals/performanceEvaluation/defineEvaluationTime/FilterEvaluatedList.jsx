...
const FilterEvaluatedList = (props) => {
    ...
    useEffect(() => {
        if (formValues.orgs && formValues.orgs != "" && fieldInfo?.positions){
            ...
            filterList.orgsFilter = (tableContent?.filter((item) => item?.subOrgPartyId.includes(formValues?.orgs) ))
    ...
    useEffect(() => {
        if (formValues.positions && formValues.positions != "[]" && fieldInfo?.positions){
            ...
            eval(formValues.positions).map((e,i)=>{
                filterData = filterData.concat(tableContent.filter((item) => item?.emplPositionIds.includes(e)))
    ...
    useEffect(() => {
        if (formValues.unit && formValues.unit != "[]"){
            ...
            eval(formValues.unit).map((e,i)=>{
                filterData = filterData.concat(tableContent.filter((item) => item?.unitPartyId.includes(e)))
    ...
    useEffect(() => {
        if (formValues.roles && formValues.roles != "[]"){
            filterList.rolesFilter = (tableContent.filter((item) => eval(formValues["roles"]).indexOf(item?.role) >= 0 ))
            setFilterList(Object.assign({},filterList))
        }
        else {
