...
export default function AccountingDocumentTemplate({scrollTop}) {
    ...
    function get_dataList() {
        axios.get("/s1/payroll/voucherTemplate").then(res => {
    ...
    function create_object() {
        axios.post("/s1/payroll/voucherTemplate", {newVoucher: formValues}).then(res=>{
            dataList.add({
                ...formValues,
                ...res.data
            })
            handle_edit(res.data)
            dispatch(setAlertContent(ALERT_TYPES.SUCCESS, "الگوی جدید سند حسابداری با موفقیت اضافه شد."))
            set_waiting(false)
        }).catch(()=>{
    ...
    function update_object() {
        axios.put("/s1/payroll/voucherTemplate", {editedVoucher: formValues}).then(()=>{
            dataList.update(formValues)
            set_action(defaultAction)
            set_formValues(formDefaultValues)
            dispatch(setAlertContent(ALERT_TYPES.SUCCESS, "ویرایش الگو با موفقیت انجام شد."))
            set_waiting(false)
        }).catch(()=>{
            dispatch(setAlertContent(ALERT_TYPES.ERROR, 'خطا در ثبت اطلاعات!'));
            set_waiting(false)
    ...
    function handle_remove(row) {
        ...
        return new Promise((resolve, reject) => {
            axios.delete(`/s1/payroll/voucherTemplate?${primaryKey}=${row[primaryKey]}`).then( () => {
                resolve()
            }).catch(()=>{
                reject()
            });
    ...
    useEffect(()=>{
        ...
        axios.get("/s1/payroll/groupAccount").then(res => {
            set_data(prevState => ({...prevState, groupDetailedAccounts: res.data.allAccount}))
        }).catch(() => {});
        axios.get("/s1/payroll/glAccount").then(res => {
            set_data(prevState => ({...prevState, glAccounts: res.data.allAccount}))
        }).catch(() => {});
        axios.get("/s1/payroll/detailAccount").then(res => {
            set_data(prevState => ({...prevState, detailedAccounts: res.data.allAccount}))
        }).catch(() => {});
        axios.get("/s1/payroll/subArticleList").then(res => {
            set_data(prevState => ({...prevState, subArticle: res.data.subArticle}))
        }).catch(() => {});
    },[])
    ...
    useEffect(()=>{
        if(formValues["severalVoucher"]==="N") {
            set_formValues(prevState => ({...prevState, partyClassificationTypeEnumId: null}))
        }
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    <React.Fragment>
                        ...
                        <AccountingDocumentTemplateArticles parentKey={primaryKey} parentKeyValue={action.payload} data={data}/>
                    </React.Fragment>
                    }
                </CardContent>
            </Card>
...
function AccountingDocumentTemplateArticles({parentKey, parentKeyValue, data}) {
    ...
    function handle_remove(row) {
        return new Promise((resolve, reject) => {
            axios.delete(`/s1/payroll/vTItem?${primaryKey}=${row[primaryKey]}`).then( () => {
                resolve()
            }).catch(()=>{
                reject()
            });
    ...
    function get_dataList() {
        axios.get("/s1/payroll/vTItem?voucherTemplateId="+parentKeyValue).then(res => {
    ...
    return (
        <Card variant="outlined">
            <TablePro
                ...
                loading={dataList.list===null}
                add="external"
                addForm={<TableForm data={data} parentKey={parentKey} parentKeyValue={parentKeyValue} articleList={dataList.list}/>}
                edit="external"
                editForm={<TableForm editing={true} data={data} parentKey={parentKey} parentKeyValue={parentKeyValue} articleList={dataList.list}/>}
                removeCallback={handle_remove}
                showRowNumber={false}
...
function TableForm({editing=false, parentKey, parentKeyValue, articleList, data,...restProps}) {
    ...
    const formStructure = [{
        ...
        type    : "select",
        options : "VTItemType",
        filterOptions: options => options.filter(o=>o["parentEnumId"]==="VTITOther"),
        ...
        optionLabelField: "title",
        display : !!formValues["vtItemTypeEnumId"]?.startsWith("VTITPayrollFactor"),// || formValues["vtItemTypeEnumId"]==="VTITPayrollFactorSave" || formValues["vtItemTypeEnumId"]==="VTITPayrollFactorArrears" || formValues["vtItemTypeEnumId"]==="VTITPayrollFactorSeverance",
        required: !!formValues["vtItemTypeEnumId"]?.startsWith("VTITPayrollFactor"),//==="VTITPayrollFactor" || formValues["vtItemTypeEnumId"]==="VTITPayrollFactorSave" || formValues["vtItemTypeEnumId"]==="VTITPayrollFactorArrears" || formValues["vtItemTypeEnumId"]==="VTITPayrollFactorSeverance"
    }]

    console.log('vliusssss',formValues["vtItemTypeEnumId"]?.startsWith("VTITPayrollFactor"))

    const handle_add = ()=>{
        let data = {...formValues , [parentKey]: parentKeyValue }
        axios.post("/s1/payroll/vTItem", { newVTItem : data}).then((res) => {
            set_waiting(false)
    ...
    const handle_edit = ()=>{
        axios.put("/s1/payroll/vTItem", {editedVTItem: formValues}).then(() => {
            set_waiting(false)
            setFormValues(formDefaultValues)
    ...
    return(
        <FormPro
            ...
            submitCallback={()=>{
                if(articleList.findIndex(i => i["seqNum"]===formValues["seqNum"] && i["vtItemId"]!==formValues["vtItemId"] )>=0) {
                    setFormValidation({
                        seqNum: {error: true, helper: "ترتیب تعیین شده برای آرتیکل تکراری است!"}
                    })
                    return false
                }
                dispatch(setAlertContent(ALERT_TYPES.WARNING, 'در حال ارسال اطلاعات...'));
