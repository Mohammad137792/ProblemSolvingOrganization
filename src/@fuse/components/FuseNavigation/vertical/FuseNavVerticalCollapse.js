...
import checkPermision from 'app/main/components/CheckPermision'
...
function FuseNavVerticalCollapse(props)
{
    ...
    const  asss = useSelector(({ fadak }) => fadak);

    const classes = useStyles(props);
    const [open, setOpen] = useState(() => needsToBeOpened(props.location, props.item));
    const {item, nestedLevel, active} = props;
    let paddingValue = 40 + (nestedLevel * 16);
    let padding = (paddingValue > 80 ? 80 : paddingValue).toString();
    const listItemPadding = nestedLevel > 0 ? 'ltr:pl-' + padding + " rtl:pr-" + padding : 'ltr:pl-24 rtl:pr-24';
    ...
    if (item?.permision && !checkPermision(item.permision , asss) ){

        // console.log("permition", checkPermision(item?.permision))
    ...
    return (
        <ul className={clsx(classes.root, open && "open")}>
            ...
            >
                {item.icon && (
                    <Icon color="action" className="text-16 flex-shrink-0 ltr:mr-16 rtl:ml-16">{item.icon}</Icon>
                )}
