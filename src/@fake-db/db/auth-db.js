    ...
    users: [
        {
            ...
            role    : "admin",
            data    : {
                'displayName': 'محمدحسین صیرفی',
                'photoURL'   : 'assets/images/avatars/seirafi.jpg',
                'email'      : 'admin',
                settings     : {
                    layout          : {
                        style : 'layout1',
...
mock.onGet('/api/auth').reply((config) => {
    ...
    const error = {
        email   : user ? null : 'نام کاربری نادرست است',
        password: user && user.password === password ? null : 'کلمه عبور نادرست است'
    };
